...
//Checkeando que los inputs sean números
...
async function get (index, years, initialDeposit, monthlyDeposit){
  try {
    ...
    $buttonCal.classList.remove('btn-loading');
    document.title = `MOkA - Fondo ${simulation.name}`;
    //returns contiene los flujos futuros de la inversión
    let returns = simulation.returns;
    //Se llama al método getHtml para renderizar la simulación
    $simulation.innerHTML = simulation.getHtml(formatNumber);
    $simulation.classList.contains('hiden-container') ? $simulation.classList.remove("hiden-container") :$simulation;
...
$buttonCal.addEventListener("click", (e) => {
  ...
  if ($initialDeposit.value && $monthlyDeposist.value) {
    $buttonCal.classList.add('btn-loading');
